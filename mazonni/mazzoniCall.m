function [ prices ] = mazzoniCall( S, K, T, t, r, w, alpha, beta, gamma, sigma)
...
%   Detailed explanation goes here
    ...
    if (alpha < 0 | w <0 | beta <0 | gamma <0 | sigma <0) 
        prices = Inf(nos,1);
        return
    end
    
%    h = var(log(S(2:numel(S))./S(1:numel(S)-1)));
    h = 0.05^2;
    
    rho = alpha*gamma^2 + beta;
    
    mu = tau.*r - (h*(rho.^tau - 1))/(2*(rho - 1)) + ((tau/2 - (rho.^tau - 1)/(2*(rho - 1)))*(alpha + w))/(rho - 1);
